...
const DocumentPanel = ({ isOpen, onClose }) => {
  ...
  const handleUploadAndAnalyze = async () => {
    ...
    try {
      ...
      const analyzeResponse = await axios.post('http://localhost:8000/analyze', { filename: filenameToSend });
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
          className="fixed top-0 right-0 h-full bg-gray-900 text-gray-200 shadow-lg z-40 p-6 flex flex-col border-l border-gray-700"
          initial={{ x: '100%' }}
          animate={{ x: 0 }}
          exit={{ x: '100%' }}
        ...
        >
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold text-cyan-400">Document Processing</h2>
            <motion.button
              onClick={onClose}
          ...
          <div
            className={`border-2 border-dashed rounded-lg p-6 text-center transition duration-200 ease-in-out flex flex-col items-center justify-center ${
              isProcessing
                ? 'border-gray-700 bg-gray-800 opacity-70 cursor-not-allowed'
                : isDragging
                  ? 'border-cyan-500 bg-gray-800 cursor-pointer'
                  : 'border-gray-700 bg-gray-800 hover:border-cyan-600 cursor-pointer'
            }`}
          ...
          >
            <FaFileUpload
              className={`text-5xl mb-4 ${isProcessing ? 'text-gray-600' : isDragging ? 'text-cyan-400' : 'text-gray-500'}`}
            />
            ...
            {!isProcessing && (
              <motion.button
                className="bg-cyan-600 hover:bg-cyan-700 text-white py-2 px-6 rounded-lg transition duration-200 ease-in-out shadow-md"
          ...
          >
            ...
            <AnimatePresence mode="wait">
              {uploadStatus && (
                <motion.p
                  key="uploadStatus"
                  className={`mt-2 flex items-center ${uploadStatus.includes('failed') ? 'text-red-400' : 'text-green-400'}`}
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  transition={{ duration: 0.3 }}
                >
                  {uploadStatus.includes('failed') ? <FaExclamationCircle className="mr-2" /> : <FaCheckCircle className="mr-2" />}
                  {uploadStatus}
                </motion.p>
              )}
              {analyzeStatus && (
                <motion.p
                  key="analyzeStatus"
                  className={`mt-2 flex items-center ${analyzeStatus.includes('failed') ? 'text-red-400' : 'text-green-400'}`}
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  transition={{ duration: 0.3 }}
                >
                  {analyzeStatus.includes('failed') ? <FaExclamationCircle className="mr-2" /> : <FaCheckCircle className="mr-2" />}
                  {analyzeStatus}
                </motion.p>
              )}
          ...
          <motion.button
            ...
            className={`mt-6 w-full bg-cyan-600 hover:bg-cyan-700 text-white py-3 rounded-lg transition duration-200 ease-in-out shadow-md flex items-center justify-center ${
          ...
          {file && !isProcessing && (
            <motion.button
              ...
              className="mt-4 w-full text-red-400 hover:text-red-500 text-sm transition duration-200 ease-in-out"
